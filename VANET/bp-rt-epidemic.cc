...
{
	m_protocolName = "RTEpidemic.buff";
}
...
LinkBundle RTEpidemic::FindNextToSend()
{
	...
	if ((m_linkManager->GetConnectedLinks().size() > 0) && (GetNBundles() > 0)) {
		...
		if (!linkBundleList.empty()) {
			m_BundleCached.push_back(linkBundleList.front().GetBundle()->GetGlobalId());
...
void RTEpidemic::SetReplicationFactor(Ptr<Bundle> bundle)
{
	//nothing
}
...
LinkBundleList RTEpidemic::GetAllBundlesForLink(Ptr<Link> link)
{
	...
	if (link->GetState() == LINK_CONNECTED) {
		for (BundleList::iterator iter = m_bundleList.begin(); iter
			...
			Bid = bundle->GetGlobalId();
			if(!BundleIsPresentInNeighbor(link->GetRemoteEndpointId(), Bid) &&
					link->GetRemoteEndpointId() == bundle->GetDestinationEndpoint()){
					direct.push_back(LinkBundle(link,*iter));
					return direct;
			}

			else if (!BundleIsPresentInNeighbor(link->GetRemoteEndpointId(), Bid)
...
void RTEpidemic::DoSendHello(Ptr<Socket> socket, BundleEndpointId eid)
{
    ...
    else
    {
    	if (m_SendNeightList) {
			/*Neigh List Type*/
			NeighListHeader header;
			header.SetBundleEndpointId(m_eid);
			TypeTag type(9);
			BundleList::const_iterator it;

			//std::cout<<"size" << m_bundleList.size() << "\n";
			for (it = m_bundleList.begin(); it != m_bundleList.end(); ++it) {
				header.InsertBundleId((*it)->GetGlobalId());
			}

			vector<uint32_t>::iterator itv;

			for (itv = m_BundleDeliverys.begin(); itv != m_BundleDeliverys.end(); ++itv) {
				header.InsertBundleId((*itv));
			}
			...
			m_SendNeightList = false;

			//std::cout<<"(" <<m_eid <<")"<< "Send Hello " << Simulator::Now().GetSeconds()<<"\n";
			socket->Send(hello);
    	}
    	else {
			/* Epidemic Type */
...
void RTEpidemic::DoHandleHello(Ptr<Socket> socket)
{
	...
	TypeTag type;
	receivedHello->PeekPacketTag(type);
	...
	if (type.GetType() == 4) {
		...
		receivedHello->PeekHeader(header);
		NS_LOG_DEBUG ("(" << m_node->GetId () << ") - eid = " << header.GetBundleEndpointId());
		m_nBundleList[header.GetBundleEndpointId()] = header.GetCopyList();
		m_BundleCached.clear();
	}
